// Assets/PF/Editor/Diagnostics/LogCategoryTreeInspector.cs
using System;
...
public class LogCategoryTreeInspector : Editor
{
    // lưu trạng thái mở/đóng foldout theo tham chiếu Node
    private static readonly Dictionary<LogCategoryTree.Node, bool> _fold = new();

    // hàng đợi thao tác sửa danh sách (tránh sửa List khi đang vẽ GUI)
    private readonly List<Action> _postOps = new();

    private LogLevel _applyDefault = LogLevel.Error; // default khi Apply
    ...
    public override void OnInspectorGUI()
    {
        ...
        _postOps.Clear();
        DrawNode(asset, parent: null, asset.Root, "");   // vẽ cây
        if (_postOps.Count > 0)
        {
        ...
        // --- Validate: đưa lên trên, tách khỏi Apply ---
            ...
            using (new EditorGUILayout.HorizontalScope())
            {
                ...
                if (parent != null)
                {
                    if (GUILayout.Button("Delete", GUILayout.Width(60)))
                    {
                        ...
                        return; // ngừng vẽ node này
                    }
                }
            }

            // BỎ hiển thị path để gọn
